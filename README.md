...
The design provides the physical interconnect, shared SPI route, separate control signals, local decoupling, power distribution and test access needed to establish whether vibration-window data can be transported from a Thin-Pod node to a Gateway host for later DSP and TinyML work.

The immediate aim is not to claim a completed monitoring product. It is to verify the Gateway PCB, establish the module-to-host data path and preserve an OSHWA-ready documentation discipline from first bring-up.
